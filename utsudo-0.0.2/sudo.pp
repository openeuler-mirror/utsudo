...
%if [deb]
	...
	$name: non-standard-file-perm etc/utsudoers 0440 != 0644
	$name: non-standard-dir-perm etc/utsudoers.d/ 0750 != 0755
...
%if [rpm]
	...
	centos*|rhel*)
		...
		if test $osrelease -lt 50; then
			cat > ${pp_destdir}/etc/pam.d/utsudo <<-EOF
			#%PAM-1.0
			...
			session    required	pam_limits.so
			EOF
			cat > ${pp_destdir}/etc/pam.d/utsudo-i <<-EOF
			#%PAM-1.0
			auth       include	sudo
